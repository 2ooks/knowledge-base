...
- **Structural credential isolation in Managed Agents:** In coupled design, prompt injection only had to convince Claude to read its own environment to get credentials; "Claude is getting increasingly smart" so narrow scoping is an assumption that goes stale; structural fix: tokens never reachable from sandbox where generated code runs ([[anthropic-scaling-managed-agents]])
- **Git token bundling pattern:** Repository access tokens clone repo during sandbox initialization and wire into local git remote; `git push` and `git pull` work without agent handling token; credentials consumed during setup, not available at runtime ([[anthropic-scaling-managed-agents]])
- **MCP OAuth vault proxy pattern:** OAuth tokens stored in secure vault outside sandbox; Claude calls MCP tools via dedicated proxy that takes session token, fetches credentials from vault, makes external service call; harness never sees credentials ([[anthropic-scaling-managed-agents]])
