...
- Continual learning for deployed agents happens at three layers: Model (weights, challenged by catastrophic forgetting), Harness (Meta-Harness pattern: run traces → evaluate → coding agent suggests changes), and Context (external config files like CLAUDE.md / SOUL.md, updateable at agent, tenant, or mixed levels) ([[langchain-continual-learning-for-ai-agents]])
- Context-layer learning tools for tenant-level updates: Hex Context Studio, Decagon Duet, Sierra Explorer; updates can happen offline ("dreaming" in OpenClaw) or in-the-hot-path while the agent runs ([[langchain-continual-learning-for-ai-agents]])
- Deep Agents (LangChain's open-source, model-agnostic base harness) supports context-layer learning in a production-ready way; all three learning flows are powered by traces collected via LangSmith ([[langchain-continual-learning-for-ai-agents]])
