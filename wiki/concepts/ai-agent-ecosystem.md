...
- **Image-to-code as new developer workflow primitive:** GPT-Image-2 positioning "image generation as a front-end for coding agents: generate a UI spec as an image, then have Codex or another code agent implement against that visual reference." Design tools (Figma, Canva, Firefly, fal, Hermes Agent) integrate GPT-Image-2, creating new workflows that may bypass traditional PR surfaces ([[ainews-openai-launches-gpt-image-2]])
- **Google Deep Research Max productizes overnight research agents:** Launched Deep Research and Deep Research Max via Gemini API (Gemini 3.1 Pro) with collaborative planning, MCP support, multimodal inputs (PDF/CSV/image/audio/video), code execution, native chart/infographic generation, and real-time progress streaming. Benchmarks: 93.3% on DeepSearchQA, 85.9% on BrowseComp, 54.6% on HLE. "Google is clearly productizing 'overnight due diligence / analyst report generation' and making MCP-backed internal data access a standard part of research agents" ([[ainews-openai-launches-gpt-image-2]])
- **vLLM deployment recipes as agent-readable infra layer:** The redesigned recipes.vllm.ai maps model pages to runnable deployment recipes, includes interactive command builder, supports NVIDIA and AMD, covers tensor/expert/data parallel variants, and exposes JSON API for agents—"exactly the kind of infra documentation layer that reduces operator friction for serving new open models" ([[ainews-openai-launches-gpt-image-2]])
