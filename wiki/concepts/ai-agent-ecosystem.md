...
- Continual learning for AI agents happens at three distinct layers: Model (updating weights via SFT/RL/GRPO), Harness (optimizing the code and instructions driving all agent instances), and Context (instance-specific instructions/skills/memory outside the harness) ([[langchain-continual-learning-for-ai-agents]])
- Model-layer continual learning faces catastrophic forgetting (models degrade on previously known tasks when updated); harness-layer learning uses Meta-Harness pattern (run agent, evaluate, store traces, use coding agent to suggest harness code changes); context-layer learning happens at agent level (OpenClaw's SOUL.md), tenant level (user/org/team via Hex Context Studio, Decagon Duet, Sierra Explorer), or mixed ([[langchain-continual-learning-for-ai-agents]])
- Context updates occur via two methods: after-the-fact offline jobs (OpenClaw "dreaming" — extracting insights from recent traces), or in-the-hot-path (agent updates memory while working); all three learning flows are powered by traces collected via LangSmith ([[langchain-continual-learning-for-ai-agents]])
- Deep Agents is LangChain's open-source, model-agnostic, general-purpose base harness supporting production-ready user-level memory, background learning, and trace-powered continual improvement (used to improve Skills on Terminal Bench) ([[langchain-continual-learning-for-ai-agents]])
