...
- GitHub allows PR disablement for first time in history (April 2026): open source repos can now disable pull requests entirely, previously only issues could be disabled; signals GitHub acknowledging git-based workflows may not be suitable for AI-native development ([[ainews-rip-pull-requests]])
- Shift from Pull Requests to Prompt Requests: Pete Steinberger and Theo advocate for "Prompt Requests" over Pull Requests citing three advantages: (1) no merge conflicts, (2) easier for maintainer to modify prompts than review code, (3) lower security risk from malicious code in PRs; Mitchell Hashimoto and Amp Code building "reputation-based" contribution systems ([[ainews-rip-pull-requests]])
- Cloudflare complete agent platform positioning: Project Think (durable execution, sub-agents, persistent sessions, sandboxed code execution, runtime tool creation), Agent Lee (in-dashboard agent using sandboxed TypeScript for prompt-driven operations), voice pipeline (real-time STT/TTS over WebSockets), Browser Run (Live View, human-in-the-loop, session recordings, CDP endpoints, WebMCP); "production agent platform is really a composition of durable runtime + UI grounding + browser + voice + sandbox" ([[ainews-rip-pull-requests]])
